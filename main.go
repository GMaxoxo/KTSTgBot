...
var gUsefulActivities = Activities{
	...
	{"swimming", "Плаванье (15 minutes)", 1},
	{"walk", "Прогулка (15 minutes)", 1},
	{"chores", "Домашние дела", 1},

	// Work
	{"work_learning", "Изучение рабочих материалов (15 минут)", 1},
	{"portfolio_work", "Работа над проектом портфолио (15 минут)", 1},
...
var gRewards = Activities{
	...
	{"watch_movie", "Просмотр фильма (1 фильм)", 30},
	{"social_nets", "Скролить социальные сети (30 минут)", 10},
...
func init() {
	_ = os.Setenv(TOKEN_NAME_IN_OS, "7047129269:AAFcktxEl6j1uj-vzZokgD0CrF0XbPGFJDE")
...
func printIntro(update *tgbotapi.Update) {
	sendMessageWithDelay(2, "Приветствую! "+EMOJI_SUNGLASSES)
...
func processUsefulActivity(activity *Activity, user *User) {
	errorMsg := ""
	if activity.coins == 0 {
		errorMsg = fmt.Sprintf(`действие "%s" не имеет определенной стоимости`, activity.name)
	...
	if errorMsg != "" {
		...
		resultMessage = fmt.Sprintf(`%s, "%s" действие завершено! %d %s был добавлен в ваш аккаунт. Продолжай в том же духе! %s%s Теперь у вас есть %d %s`,
...
func updateProcessing(update *tgbotapi.Update) {
	...
	default:
		...
		msg := fmt.Sprintf("%s, Извините, я не распознал код '%s' %s Пожалуйста, сообщите об этой ошибке моему создателю.", user.name, choiceCode, EMOJI_SAD)
		sendStringMessage(msg)
	}
}
